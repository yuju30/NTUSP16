...
static void *threadpool_thread(void* server)
 {
     while(1){
         pthread_mutex_lock(&(pool->lock));

         while(pool->todo_fd==-1){
          pthread_cond_wait(&(pool->notify), &(pool->lock));
         }
         int todo_fdtemp = pool->todo_fd;
         pool->todo_fd=-1;  
         pool->thread_count++;         
         int j; 
          for ( j = 0; j < 20; ++j){
           if(pool->dealfd[j]==-1){
              pool->dealfd[j]=todo_fdtemp;break;
           }
          }

         pthread_mutex_unlock(&(pool->lock));
          
          handle_request(server,todo_fdtemp);
...
int csiebox_server_run(csiebox_server* server) { 
  ...
  events = calloc(MAXEVENTS, sizeof(event));
    ...
    pool->threads = (pthread_t *) malloc(sizeof(pthread_t) * (server->arg.thread_num));     
    pool->todo_fd=-1;
    pool->thread_count=0;
    memset(pool->dealfd,-1,sizeof(pool->dealfd));
    
    pthread_mutex_init(&(pool->lock),NULL);
    pthread_cond_init(&(pool->notify),NULL);

    for (int i = 0; i < server->arg.thread_num; ++i){
    pthread_create(&(pool->threads[i]), NULL, threadpool_thread, (void *)server);
    }
  ...
  while (1) {
    ...
    // handle request from connected socket fd
       //handle_request(server,events[i].data.fd);
            ...
            int deal=0;
            for (int j = 0; j < 20; ++j){
               if(pool->dealfd[j]==events[i].data.fd){
                  deal=1;break;
               }
              }
         
           if(!deal){
            if(pool->thread_count >= server->arg.thread_num){
                csiebox_protocol_header busy;
                memset(&busy, 0, sizeof(csiebox_protocol_header));
                busy.res.magic = CSIEBOX_PROTOCOL_MAGIC_REQ;
                busy.res.op = CSIEBOX_PROTOCOL_OP_SYNC_META;
                busy.res.datalen = sizeof(busy);
                busy.res.status = CSIEBOX_PROTOCOL_STATUS_BUSY;
                send_message(events[i].data.fd, &busy, sizeof(busy));
            }//busy
            else{
              pool->todo_fd =  events[i].data.fd;
...
static int parse_arg(csiebox_server* server, int argc, char** argv) {
  ...
  while ((keylen = getdelim(&key, &keysize, '=', file) - 1) > 0) {
    ...
    if (strcmp("path", key) == 0) {
      ...
      if (vallen <= sizeof(server->arg.thread_num)){
        ...
        if (complete_message_with_header(conn_fd, &header, &meta)) {
           ...
           if(S_ISREG(meta.message.body.stat.st_mode)){
              ...
              if(fl!=0){
                 ...
                 retheader.res.magic = CSIEBOX_PROTOCOL_MAGIC_RES; 
                 retheader.res.op = CSIEBOX_PROTOCOL_OP_SYNC_META;
                 retheader.res.status = CSIEBOX_PROTOCOL_STATUS_LOCK;
                 if(!send_message(conn_fd, &retheader, sizeof(retheader))){
                  fprintf(stderr, "send lock message fail\n");
                 }
                 flock(fd,LOCK_UN);
                 return; 
              }
