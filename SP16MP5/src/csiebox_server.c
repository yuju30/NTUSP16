...
{
  uint32_t working_thread = run_num;
...
static int parse_arg(csiebox_server* server, int argc, char** argv) {
  ...
  while ((keylen = getdelim(&key, &keysize, '=', file) - 1) > 0) {
    ...
    if (strcmp("path", key) == 0) {
     ...
     pid_t g_pid = getpid();
     char f_path[1000];
     memset(f_path, 0, 1000);
     sprintf(f_path,"%s/csiebox_server.%d",val,g_pid);
     sprintf(rpath,"%s",f_path);
     mkfifo(f_path,0777);
     char p_path[1000];
     memset(p_path, 0, 1000);
     sprintf(p_path,"%s/csiebox_server.pid",val);
     FILE* fd_c;
     fd_c=fopen(p_path,"w+");
     fprintf(fd_c,"%d", g_pid);
     accept_config[3] = 1;
     fclose(fd_c);  
    } 
